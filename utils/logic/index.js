...
class Logic {
  ...
  unzip(FROM, DIRECTORY_NAME, commandsFileName) {
    ...
    const TO = oldDIr + "/" + DIRECTORY_NAME;
    const spinner = ora("Doing something useful...!");
    spinner.start();
    let message;

    // process.chdir(TO);
    _7zip.unpack(FROM, TO, (err) => {
      if (err) {
        ...
        let commandsFilePath = path.join(
          process.cwd(),
          "src",
          "commands",
          commandsFileName
        );
        let targetCommandsFilePath = path.join(
          process.cwd(),
          DIRECTORY_NAME,
          "spin.commands.md"
        );
  ...
  async createCommandsFile(SOURCE_DIRECTORY, DIRECTORY_TARGET) {
    ...
    fileWriteStream.on("error", (err) =>
      console.log("Write Commands Error: ", err)
    );
